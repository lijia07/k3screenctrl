...
if [ -z "$(iptables --list | grep UPSP)" -a -z "$(iptables --list | grep DWSP)" ]; then
	iptables -N UPSP
	iptables -N DWSP
	mkdir /tmp/k3screenctrl/lan_speed
fi
for ((i=0;i<${#arp_ip[@]};i++))
do
	if [ -z "$(iptables -nvx -L FORWARD | grep DWSP | grep ${arp_ip[i]} -w)" -a -z "$(iptables -nvx -L FORWARD | grep UPSP | grep ${arp_ip[i]} -w)" ]; then
		...
		echo $(date +%s) > /tmp/k3screenctrl/lan_speed/${arp_ip[i]}
		echo 0 >> /tmp/k3screenctrl/lan_speed/${arp_ip[i]}
		echo 0 >> /tmp/k3screenctrl/lan_speed/${arp_ip[i]}
	fi
done
...
do
	...
	last_speed_dw=$(cut -d$'\n' -f,3 /tmp/k3screenctrl/lan_speed/${online_list[i]})
	...
	echo $now_speed_time > /tmp/k3screenctrl/lan_speed/${online_list[i]}
	echo $now_speed_up >> /tmp/k3screenctrl/lan_speed/${online_list[i]}
